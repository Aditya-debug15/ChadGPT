...
with col2:
    # Create 3 tabs
    tab1, tab2= st.tabs(["Token", "Analysis"])

    with tab1:
        st.markdown("### Steps to generate token")
        st.markdown("1. Select your land using the draw tool on the map")
        st.markdown("2. Click on the export button on the map to download the geojson file")
        st.markdown("3. Upload the geojson file here")
        uploaded_file = st.file_uploader("Choose a geojson file")
        st.markdown("4. After uploading the file, click on predict yield button. A token will be generated if the yeild satisfies a certain threshold value (To be implemented)")
        if st.button("Predict yield"):
            if uploaded_file is not None:
                # read the uploaded json file
                data = json.load(uploaded_file)
                geo = data["features"][0]["geometry"]
                # calculate the area of the selected land
                area = ee.Geometry(geo).area(1).getInfo()
                # calculate the area in km2
                area = area / 1000000
                st.write("Area of the selected land is", area, "**square kilometers**")
                # Logic to predict yeild
                st.write("Predicted yield is **xy quintals per hectare**")
                st.write("Your token is **1234567890**")
